...
data class FarmContext(val region:String,val stage:String,val tempMin:Double?=null,val tempMax:Double?=null,val humidity:Double?=null,val weatherWarning:Boolean=false,val cough:Boolean=false,val diarrhea:Boolean=false,val intakeDrop:Boolean=false,val mortalityRise:Boolean=false,val diseaseNearby:Boolean=false,val priceChangePct:Double?=null,val shipmentDueDays:Int?=null,val recentFcr:Double?=null,val baselineFcr:Double?=null,val signal:TimedSignal?=null)
data class ActionStep(val order:Int,val title:String,val detail:String,val professional:Boolean=false)
data class DecisionCard(val domain:RiskDomain,val score:Int,val title:String,val why:List<String>,val observe:List<String>,val measure:List<String>,val firstAdjustment:String,val differentials:List<String>,val escalation:String,val evidence:EvidenceLevel,val steps:List<ActionStep>)
...
object FarmDecisionEngine{
 ...
 private fun card(domain:RiskDomain,raw:Double,title:String,why:List<String>,observe:List<String>,measure:List<String>,adjust:String,diff:List<String>,escalate:String,evidence:EvidenceLevel,signal:TimedSignal?):DecisionCard{
  val score=(raw*reliability(signal)).toInt().coerceIn(0,100)
  return DecisionCard(domain,score,title,why,observe,measure,adjust,diff,escalate,evidence,listOf(ActionStep(1,"관찰",observe.joinToString(" · ")),ActionStep(2,"측정",measure.joinToString(" · ")),ActionStep(3,"우선 조정",adjust),ActionStep(4,"지속 시 감별",diff.joinToString(" · ")+" — "+escalate,true)))
 }
