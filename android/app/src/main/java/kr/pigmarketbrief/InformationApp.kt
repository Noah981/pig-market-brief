...
@Composable fun DondonApp(){
 ...
 Scaffold(bottomBar={NavigationBar{listOf("오늘","돈가","시장","혜택","더보기").forEachIndexed{i,label->NavigationBarItem(selected=tab==i,onClick={tab=i;detail=""},icon={Text(listOf("⌂","↗","▥","◇","⋯")[i],fontSize=24.sp)},label={Text(label,fontSize=14.sp)})}}}){padding->
 Column(Modifier.padding(padding).fillMaxSize().verticalScroll(remember(tab,detail){androidx.compose.foundation.ScrollState(0)}).padding(20.dp),verticalArrangement=Arrangement.spacedBy(16.dp)){
  ...
  if(data.stale)Notice("마지막 저장 데이터\n돈가 기준 ${formatDay(data.pig.date)} · 갱신 ${data.pig.updatedAt.ifBlank{"기록 없음"}}")
  when(detail){
   ...
   "인증"->CertificationHub()
    ...
    else->{Heading("필요할 때 간단하게","매일 입력하지 않아도 괜찮습니다");listOf("질병" to "국내외 질병·방역","정산" to "출하 정산 예측","사료" to "사료 주문시기","프로필" to "농장 지역·조건","인증" to "우리 농장 인증","지역" to "GPS 현재 위치","알림" to "알림 설정").forEach{(target,title)->Tile(title,"열기"){open(target)}}}
   }
  }
 }
 }
...
@Composable fun Tile(title:String,subtitle:String,onClick:()->Unit){Card(Modifier.fillMaxWidth().clickable(onClick=onClick),shape=RoundedCornerShape(22.dp)){Column(Modifier.padding(20.dp),verticalArrangement=Arrangement.spacedBy(8.dp)){Text(title+"  ›",fontSize=21.sp,fontWeight=FontWeight.Bold);Text(subtitle,fontSize=16.sp)}}}
