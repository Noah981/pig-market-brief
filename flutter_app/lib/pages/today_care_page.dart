...
class _TodayCarePageState extends State<TodayCarePage>{
  ...
  @override Widget build(BuildContext context){
    final w=widget.guide,risks=WeatherFarmRepository().risks(w);
    return PageShell(title:'오늘 관리',subtitle:'날씨에 맞춰 질병 신호와 환기·시설을 함께 확인하세요',child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
      _weather(w),const SizedBox(height:14),_title('오늘 주의할 건강 신호',Icons.health_and_safety_outlined),
  ...
  Widget _weather(WeatherFarmGuide w)=>Container(padding:const EdgeInsets.all(14),decoration:appCard(color:AppColors.lightBlue,radius:18),child:Column(children:[
    Row(children:[const Text('🌤️',style:TextStyle(fontSize:30)),const SizedBox(width:8),Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text(w.region,style:const TextStyle(fontSize:11,color:AppColors.secondary)),Text('${w.tempMax.toStringAsFixed(0)}°C · ${w.condition}',style:const TextStyle(fontSize:23,fontWeight:FontWeight.w900))])),IconButton(onPressed:widget.onRefresh,icon:const Icon(Icons.refresh,color:AppColors.blue))]),
