...
class _TodayCarePageState extends State<TodayCarePage>{
  ...
  Widget _weather(WeatherFarmGuide w)=>Container(padding:const EdgeInsets.all(14),decoration:appCard(color:AppColors.lightBlue,radius:18),child:Column(children:[
    Row(children:[const Icon(Icons.wb_sunny_rounded,size:48,color:Color(0xFFFFB300)),const SizedBox(width:10),Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[InkWell(onTap:_pickRegion,child:Row(children:[const Icon(Icons.location_on,color:AppColors.blue,size:16),const SizedBox(width:3),Flexible(child:Text(FarmLocationSettings.instance.location.label,style:const TextStyle(fontSize:12,fontWeight:FontWeight.w700))),const Icon(Icons.expand_more,size:16,color:AppColors.secondary)])),Row(crossAxisAlignment:CrossAxisAlignment.end,children:[Text('${w.tempMax.toStringAsFixed(0)}°C',style:const TextStyle(fontSize:36,height:1.1,fontWeight:FontWeight.w900)),const SizedBox(width:7),Padding(padding:const EdgeInsets.only(bottom:5),child:Container(padding:const EdgeInsets.symmetric(horizontal:9,vertical:4),decoration:BoxDecoration(color:AppColors.lightCoral,borderRadius:BorderRadius.circular(16)),child:Text(w.condition,style:const TextStyle(fontSize:11,fontWeight:FontWeight.w800,color:AppColors.coral))))])])),IconButton(onPressed:widget.onRefresh,icon:const Icon(Icons.refresh,color:AppColors.blue))]),
