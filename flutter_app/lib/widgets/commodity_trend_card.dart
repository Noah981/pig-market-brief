...
class CommodityTrendCard extends StatelessWidget {
  const CommodityTrendCard({super.key,required this.items});
  ...
  Widget build(BuildContext context)=>Container(padding:const EdgeInsets.fromLTRB(13,12,13,13),decoration:appCard(color:const Color(0xFFFFF3F6),radius:20),child:Column(children:[
    ...
    SizedBox(height:112,child:ListView.separated(scrollDirection:Axis.horizontal,itemCount:items.length,separatorBuilder:(_,__)=>const SizedBox(width:7),itemBuilder:(context,i){final x=items[i];final up=x.change>=0;return Container(width:105,padding:const EdgeInsets.all(10),decoration:BoxDecoration(color:Colors.white,borderRadius:BorderRadius.circular(13)),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Row(children:[Icon(x.icon,size:22,color:i==0?AppColors.green:i==2?const Color(0xFF293249):const Color(0xFFE5A62B)),const SizedBox(width:5),Expanded(child:Text(x.name,style:const TextStyle(fontSize:10.5,fontWeight:FontWeight.w800),maxLines:2))]),const Spacer(),Text('${x.value} ${x.unit}',style:const TextStyle(fontSize:10.5,fontWeight:FontWeight.w800),maxLines:1),const SizedBox(height:3),Text('${up?'▲':'▼'} ${x.change.abs().toStringAsFixed(1)}%',style:TextStyle(fontSize:10,fontWeight:FontWeight.w800,color:up?AppColors.coral:AppColors.blue))]));})),
  ]));
}
