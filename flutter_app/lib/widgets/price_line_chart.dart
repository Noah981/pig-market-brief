...
class PriceLineChart extends StatelessWidget {
  const PriceLineChart({super.key, required this.series,this.height=102});
  final PriceSeries series;
  final double height;
  @override
  Widget build(BuildContext context) {
    final values=series.values;
    if(values.length<2)return SizedBox(height:height,child:const Center(child:Text('해당 기간의 공식 이력을 확인 중입니다.',style:TextStyle(fontSize:9,color:AppColors.secondary))));
    ...
    titlesData: FlTitlesData(topTitles: const AxisTitles(), rightTitles: const AxisTitles(),
      ...
      bottomTitles: AxisTitles(sideTitles: SideTitles(showTitles:true,interval:1,reservedSize:height<90?16:20,getTitlesWidget:(v,_){final i=v.toInt();if(i<0||i>=series.points.length)return const SizedBox();final step=series.points.length>6?2:1;return i%step==0||i==series.points.length-1?Padding(padding:const EdgeInsets.only(top:2),child:Text(_label(series.points[i].date),style:const TextStyle(fontSize:7,color:AppColors.secondary))):const SizedBox();}))),
