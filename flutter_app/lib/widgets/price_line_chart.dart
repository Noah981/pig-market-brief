...
class PriceLineChart extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => SizedBox(height: 102, child: LineChart(LineChartData(
    minY: 4000, maxY: 8000,
    ...
    titlesData: FlTitlesData(topTitles: const AxisTitles(), rightTitles: const AxisTitles(),
      leftTitles: AxisTitles(sideTitles: SideTitles(showTitles: true, reservedSize: 32, interval: 1000, getTitlesWidget: (v, _) => Text(v.toInt().toString().replaceFirst(RegExp(r'000$'), ',000'), style: const TextStyle(fontSize: 7, color: AppColors.secondary)))),
      bottomTitles: AxisTitles(sideTitles: SideTitles(showTitles: true, interval: 1, reservedSize: 18, getTitlesWidget: (v, _) { final i=v.toInt(); return i>=0&&i<8 ? Padding(padding: const EdgeInsets.only(top: 3), child: Text('9/${11+i}', style: const TextStyle(fontSize: 7, color: AppColors.secondary))) : const SizedBox();}))),
