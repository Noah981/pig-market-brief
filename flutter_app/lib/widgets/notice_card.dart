...
class NoticeCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(children: [
      const Row(children: [
        Icon(Icons.campaign, color: AppColors.coral, size: 22),
        SizedBox(width: 7),
        Expanded(child: Text('양돈 이슈 & 공지', style: TextStyle(fontSize: 19, fontWeight: FontWeight.w900))),
        Text('더보기 ›', style: TextStyle(fontSize: 11, color: AppColors.secondary)),
      ]),
      ...
      ...items.map((x) => InkWell(
        ...
        child: Padding(
          ...
          child: Row(children: [
            Container(width: 42, alignment: Alignment.center, padding: const EdgeInsets.symmetric(vertical: 3), decoration: BoxDecoration(color: x.color, borderRadius: BorderRadius.circular(8)), child: Text(x.category, style: const TextStyle(fontSize: 9.5, fontWeight: FontWeight.w700))),
            const SizedBox(width: 8),
            Expanded(child: Text(x.title, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 10.5))),
            const SizedBox(width: 7),
            Text(x.date, style: const TextStyle(fontSize: 9, color: AppColors.secondary)),
          ]),
        ),
      )),
    ]),
