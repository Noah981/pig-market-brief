  ...
  Widget build(BuildContext context) {final settings=DisplaySettings.instance;return SizedBox(
        ...
        child: Row(children: [
          ...
          Expanded(child:Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [
            ...
            SizedBox(height:settings.largeTextMode?44:settings.textScale>1?40:30,child:OutlinedButton.icon(key:const ValueKey('large_text_mode_button'),onPressed:()=>settings.setLargeTextMode(!settings.largeTextMode),icon:Icon(settings.largeTextMode?Icons.text_decrease:Icons.text_increase,size:13),label:FittedBox(child:Text(settings.largeTextMode?'기본 글씨':'큰글씨 모드')),style:OutlinedButton.styleFrom(padding:const EdgeInsets.symmetric(horizontal:7),minimumSize:Size(0,settings.largeTextMode?44:settings.textScale>1?40:30),tapTargetSize:MaterialTapTargetSize.shrinkWrap,textStyle:const TextStyle(fontSize:9,fontWeight:FontWeight.w900),foregroundColor:AppColors.coral,side:const BorderSide(color:AppColors.coral)))),
          ])),
