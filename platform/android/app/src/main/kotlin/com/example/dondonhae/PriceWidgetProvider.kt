...
object PriceWidgetStore{
 ...
 fun render(c:Context){
  ...
  val small=manager.getAppWidgetIds(ComponentName(c,PriceWidgetSmallProvider::class.java));for(id in small)manager.updateAppWidget(id,views(c,R.layout.widget_price_small,price,change,date,false,manager.getAppWidgetOptions(id)))
  val wide=manager.getAppWidgetIds(ComponentName(c,PriceWidgetWideProvider::class.java));for(id in wide)manager.updateAppWidget(id,views(c,R.layout.widget_price_wide,price,change,date,true,manager.getAppWidgetOptions(id)))
 }
 private fun views(c:Context,layout:Int,price:Int,change:Int,date:String,wide:Boolean,options:android.os.Bundle)=RemoteViews(c.packageName,layout).apply{
